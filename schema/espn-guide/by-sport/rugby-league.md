# ESPN API — rugby-league

> Per-sport slice of the [observed data guide](../index.md): what ESPN actually serves for **rugby-league**, and which endpoint answers each need. Built from 82 real rugby-league responses — OBSERVED live, not documented. Regenerate: `node schema/tools/rollup.mjs`.
...
|---|---|---|
| `headToHead · numeric · team` | 1 | `3` |

**Crawled for this guide** (1): `rugby-league/3`. The evidence below is from these leagues; other rugby-league leagues in the registry inherit the same shape.

## Which endpoint to use

URL templates use `rugby-league/3` as a representative league. Swap in any league key from the table above.

| need | endpoint | status | URL template | fields |
|---|---|---|---|---|
| Scores & live state | `scoreboard` | ✅ 14/14 | `https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/scoreboard` | [guide](../scoreboard.md) |
| Rich game detail | `summary` | ✅ 4/4 | `https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/summary?event={eventId}` | [guide](../summary.md) |
| League table | `standings` | ✅ 2/2 | `https://site.api.espn.com/apis/v2/sports/rugby-league/3/standings` | [guide](../standings.md) |
| Team directory | `teams` | ✅ 1/1 | `https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/teams` | [guide](../teams.md) |
| Roster | `team-roster` | ✅ 2/2 | `https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/teams/{teamId}/roster` | [guide](../team-roster.md) |
| Schedule | `team-schedule` | ❌ not served | `https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/teams/{teamId}/schedule` | [guide](../team-schedule.md) |
| Team season stats | `team-stats` | ❌ not served | `https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/teams/{teamId}/statistics` | [guide](../team-stats.md) |
| News | `news` | ✅ 1/1 | `https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/news?limit=5` | [guide](../news.md) |
| Injuries | `injuries` | ✅ 1/1 | `https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/teams/{teamId}/injuries` | [guide](../injuries.md) |
...
57 of the core resource shapes were reachable for rugby-league by following `$ref` links from the league root and its events (full matrix: [index.md](../index.md#core-api-resource-graph)). High-value shapes:
...
Value-bearing fields observed for rugby-league that are **not** near-universal — the sport's fingerprint. **presence** = % of this sport's crawled responses (or parent objects) that carry the field (rugby-league-specific). Field paths, types, and presence are rugby-league-only; the **value/example samples are pooled** across the handful of sports that share a field (a nested `period` may show golf's "18" next to a fight's "1–3"), so read them as shape hints. The full per-sport field tables live in the per-endpoint guides linked above.
...
| field | type | presence | values / examples |
|---|---|---|---|
| `headToHeadGames[].events[].week` | number | 100% | e.g. 9, 27, 15, 12 |
| `header.competitions[].details[].text` | string | 37% | e.g. "Paul Alamoti converts for Penrith Panthers", "Nick Meaney converts for Melbourne Storm", "Adam Reynolds converts for Brisbane Broncos", "Dylan Edwards claims a try for Penrith Panthers." |
...
| `boxscore.players[].statistics[].athletes[].athlete.displayHeight` | string | 89% | e.g. "6' 0"", "6' 2"", "6' 3"", "5' 10"" |
| `boxscore.players[].statistics[].athletes[].athlete.displayWeight` | string | 89% | e.g. "231 lbs", "211 lbs", "242 lbs", "238 lbs" |
| `boxscore.players[].statistics[].athletes[].athlete.height` | number | 89% | e.g. 72.04724409448819, 72.83464566929133, 74.01574803149606, 75.19685039370079 |
| `boxscore.players[].statistics[].athletes[].athlete.weight` | number | 89% | e.g. 231.48537529412147, 211.64377169748246, 242.50848840336533, 238.0992431596678 |
| `boxscore.players[].statistics[].athletes[].athlete.birthPlace.country` | string | 62% | values: "Australia" (63), "Ireland" (23), "New Zealand" (20), "Wales" (20), "England" (15), "Scotland" (13), "France" (6), "Italy" (5), "South Africa" (2), "Canada" (1), "New Caledonia" (1), "Papua New Guinea" (1), "Tonga" (1), "Fiji" (1) |
| `boxscore.players[].statistics[].athletes[].athlete.birthPlace.city` | string | 49% | e.g. "Sydney", "Dublin", "Auckland", "Swansea" |
| `rosters[].roster[].position.id` | string | 26% | e.g. "UKN", "BL", "20", "53" |
