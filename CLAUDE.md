...
The route surface (all under `/v1`, all coalesced behind the Cache API): `health`, `catalog` (1h TTL), `overview` (Leagues-list season pulse — fans out one cheap scoreboard fetch per league, capped at `OVERVIEW_FETCH_CAP=48` to stay under Cloudflare's 50-subrequest limit, flat 5m TTL), `scores/{sport}/{league}` (15s live / 5m idle), `summary/{sport}/{league}/{eventId}` (rich detail, 20s live / 5m idle), `standings/{sport}/{league}` (1h), `teams/{sport}/{league}` (favorites picker, 1-day TTL), `team/{sport}/{league}/{teamId}` (one favorite's live/last/next card, 15s/5m).

Why it survives the free tier (see `SPEC.md` §11): use **Cache API, not KV** (KV's 1k-writes/day cap dies at a 15s refresh; Cache API has no write cap). Serve stale instantly, refresh once in `ctx.waitUntil()` — all users of a league share that single upstream fetch. TTL follows the data: **15s when `anyLive`, 5m when idle** (the worker reads `anyLive` off its own normalized payload). Debug via the `x-cache: HIT|STALE|MISS|REVALIDATE` response header.
...
`DISPLAY-SPEC.md` governs the game-detail screen and tags every section `[cheap]` (scoreboard) vs `[rich]` (summary). **Note:** per-quarter line scores for NBA/NFL/NHL are rich-tier only (they live in summary, not the scoreboard) — do not promise them in the cheap pass.
