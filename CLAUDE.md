...
npm run capture-extra  # (re)capture the team-schedule/roster/MMA-core/racing/venue/
                       ...
                       #   tournaments — e.g. `--only odds venues leaders
                       #   standingsRecords standingsNotes situation tournaments`.
...
- `api.dart` — composes `espn_client` + the normalizers behind the SAME public method surface the app always used (`scores`/`summary`/`standings`/`catalog`/`teamCard`/`teamDetail`/`overview`/…), plus the Phase 4/5 additions: `athleteProfile` (player page), `teamLeaders` (team-page leaders card + standings sub-records), `venueFacts`/`circuitFacts` (the Venue/Circuit tab), `tournament` (the bracket screen's groups/rounds/pools/series), `competitionOdds` (core moneyline, on detail open), and `coverage` (the date-strip has-games dots). Golf `meta.golf` (core tournament fetch, best-effort) and the MMA `bouts` build (ESPN's site summary 404s for MMA → built from core per-bout status/linescores) live here as per-endpoint enrichments. `overview` fans out one cheap scoreboard per league with a concurrency cap of 8, capped at 48 leagues.
...
- `design-mirror/` — ground-truth screen mockups (`*.dc.html`) mirrored from the Claude Design project `8c0e9190` ("Sports App Design Concept"), pulled lazily as phases needed them (`Index.dc.html` manifest, `HomeFeed`, `Standings`, `TeamPlayer`, `Venue`, `Circuit`, `Tournaments`, plus `SKILL.md`/`readme.md` describing the extracted system). **`DesignSync` fetches only run from the orchestrator session** — a sub-agent workflow can read the already-mirrored files but can't pull new ones; `app/DESIGN.md` (distilled from these) remains the thing to read for actually building UI.
