...
// absent — e.g. a normal local checkout — we fall back to the debug key so
// `flutter run` / `flutter build apk` keep working without any secrets. CI writes
// key.properties from GitHub Actions secrets before building (see
...
android {
    namespace = "dev.philscott.scores"
    ...
    signingConfigs {
        create("release") {
            if (keystorePropertiesFile.exists()) {
                ...
                // storeFile resolves relative to this module dir (android/app/).
    ...
    buildTypes {
        release {
            ...
            // otherwise fall back to the debug key so local builds still run.
