...
void _showSheet(
  ...
  // Capture the navigator so the "view team page" row can close the sheet and
  // then push onto the page below it (pushing before the pop would let the pop
  // eat the team page).
  ...
  showModalBottomSheet<void>(
    ...
    builder: (_) => _FollowSheet(
      ...
      onViewTeam: (t) {
        nav.pop();
        openTeamPage(nav.context, league,
            teamId: t.teamId, name: t.name, color: t.colorHex);
      },
    ),
  );
}
...
class _FollowSheet extends ConsumerWidget {
  ...
  final void Function(_SheetTeam) onViewTeam;
  const _FollowSheet({
          ...
          children: [
            ...
            if (teams.isNotEmpty) ...[
              Container(
                ...
                child: Row(children: [
                  // §5 sheet header: dark team-tinted avatar + 2px team ring.
                  TintedAvatar(teams.first.abbr, teams.first.color,
                      size: 44, ring: true),
            ...
            for (final t in teams) ...[
              ...
              _SheetRow(
                icon: const Icon(Icons.assignment_outlined,
                    color: T.textDim, size: 22),
                title: 'View ${_short(t.name)} page',
                subtitle: 'Schedule, roster, standing, and stats',
                closeOnTap: false, // onViewTeam pops the sheet itself
                onTap: () => onViewTeam(t),
              ),
            ],
            _SheetRow(
